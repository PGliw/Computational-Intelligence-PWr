...
GA.run.experiment <- function(function.name, iterations.count) {
  ...
  # GA.best <- NULL # best result
    param.score <- GA.run.iterations(function.name, iterations.count)
  list(best.scores = param.score$best.mean, mean.scores = param.score$mean.mean)
}

# runs experiments as a batch
GA.run.experiment.list <- function(function.name, iterations.count = 10) {
    param.score.range <- GA.run.experiment(function.name, iterations.count)
  print(param.score.range$mean.scores)
    scores.plot(
      x = 1,
...
GA.run.experiment.list(function.name, 10)
